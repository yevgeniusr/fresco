...
import { redirectWith, safeReturnTo } from "@/lib/http";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!citizen) return NextResponse.redirect(new URL(`/login?returnTo=${encodeURIComponent(returnTo)}`, request.url), 303);
